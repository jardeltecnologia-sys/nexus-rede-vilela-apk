...
Write-Host " Nexus Assistente Virtual APK - Envio para GitHub" -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
Write-Host ""

$Remote = "https://github.com/jardeltecnologia-sys/nexus-rede-vilela-apk.git"
...
if (!(Test-Path ".github\workflows\android-build.yml")) {
  Write-Host "Erro: rode este script dentro da pasta nexus-rede-vilela-apk." -ForegroundColor Red
  exit 1
}
...
if ($hasChanges) {
  git commit -m "fix: create Nexus virtual assistant APK only"
} else {
...
Write-Host "Abra: https://github.com/jardeltecnologia-sys/nexus-rede-vilela-apk/actions" -ForegroundColor Green
